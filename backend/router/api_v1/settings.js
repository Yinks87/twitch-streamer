...
settingsRouter.post('/settings', requireAuth, (req, res) => {
  ...
  if (audioBitrateKbps !== undefined && (!Number.isInteger(audioBitrateKbps) || audioBitrateKbps < 32 || audioBitrateKbps > 320))
    return res.status(400).json({ error: 'audioBitrateKbps must be an integer between 32 and 320' });
  if (streamFps !== undefined && (!Number.isInteger(streamFps) || streamFps < 1 || streamFps > 120))
    return res.status(400).json({ error: 'streamFps must be an integer between 1 and 120' });
  if (restartIntervalSeconds !== undefined && (!Number.isInteger(restartIntervalSeconds) || restartIntervalSeconds < 0 || restartIntervalSeconds > 604800))
    return res.status(400).json({ error: 'restartIntervalSeconds must be an integer between 0 and 604800' });
  if (restartDelaySeconds !== undefined && (!Number.isInteger(restartDelaySeconds) || restartDelaySeconds < 0 || restartDelaySeconds > 3600))
